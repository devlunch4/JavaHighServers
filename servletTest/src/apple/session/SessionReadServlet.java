...
public class SessionReadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.println("<h2>저장된 Session 데이터가 확인 되었습니다.</h2><br><br>");
		out.println("<hr>");
		...
		// ==> 이 메서드의 반환값은 Object형이라 형변환을 해서 사용해야한다.
		String sessionValue = (String) session.getAttribute("testSession");

		if (sessionValue == null) {
			out.println("<h3>키 값이 'testSession'인 세션값이 없습니다.</h3>");
		} else {
		...
		out.println("<h3>기타 세션 관련 정보들 확인하기</h3>");
		// 세션 ID ==>세션을 구분하기 위한 고유한 값(서버에 의해 자동 생성된다.)
		out.println("세션 ID : " + session.getId() + " <br>");
		// 세션 생성시간 ==> 1970년1월1일부터 세션이 만들어진 날짜 및 시간까지의 경과시간(단위 : 밀리 세컨드)
		...
		// 세션 유효 시간 ==> '최근 세션 접근 시간' 이후로 세션 유효시간이 경과하면 세션이 자동으로 삭제된다. (기본값 : 30분)
		// 세션 유효시간 설정하기 ==> session객체.setMaxInactiveInterval(초단위 시간);
		out.println("세션 유효 시간 : " + session.getMaxInactiveInterval() + "<br>");

		// 세션 유효시간을 web.xml 에 설정하는 방법
		// <session-config>
