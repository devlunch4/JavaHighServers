...
// 2. 컨테이너는 web.xml 에 정의된 url패턴을 확ㅇ니하여 어느 서블릿을 통해서 처리해야 할 지를 검색한다.
////(로딩이 안된 경우에는 로딩을 한다.이 떄 init()메서드가 호출된다.)
////(Servlet버전 3.0이상에서는 애노테이션(@WebServlet)으로 url패턴을 설정할수 있다.
...
// 5. 요청 처리가 완료되면 HttpServletRequest객체와 HttpServletResponse 객체는 소멸된다.
...
public class ServletTest03 extends HttpServlet {
	...
	public void init() throws ServletException {
		/*
		 * // TODO Auto-generated method stub super.init();
		 */
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 방법 2 ==> 클라이언트의 전송방식(GET,POST등 )을 구분해서 직접 메서드 호출
		/*
		 * String method = req.getMethod(); System.out.println("method => " + method);
		 * 
		 * if("get".equals(method)) { this.doGet(req, resp); }else { this.doPost(req,
		 * resp); }
		 */
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		out.println("<html><head><meta charchet='UTF-8'></head><body><h1>doGet메서드 처리내용<br></h1></body></html>");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		resp.setContentType("text/html; charset=utf-8");
		PrintWriter out = resp.getWriter();
		out.println("<html><head><meta charchet='UTF-8'></head><body><h1>doPost메서드 처리내용<br></h1></body></html>");
	}
