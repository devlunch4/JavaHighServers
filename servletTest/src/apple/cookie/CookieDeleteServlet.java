...
public class CookieDeleteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 저장된 쿠키정보 삭제하기
		// 1. 쿠키데이터의 삭제는 쿠키의 남은 수명을 0으로 설정하는 방법을 사용한다.
		// ==> 쿠키의 수명은 쿠키를 저장하는 addCookie()메서드를 호출하기 전에 해당쿠키 객체의 setMaxAge()메서드를 사용해서
		// 설정한다.
		...
		if (cookieArr == null || cookieArr.length == 0) {
			...
			for (Cookie cookie : cookieArr) {
				...
				if ("addr".equals(name)) {// 삭제할 쿠키변수인지 확인
					...
					// 이때 '쿠키값'은 아무거나 설정해도 된다.(이유 : 차피 삭제될 데이터 이기 떄문이다.)
					Cookie delCookie = new Cookie("addr", "test");
					delCookie.setMaxAge(0); //유지 시간은 0으로 설정
