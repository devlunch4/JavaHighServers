...
public class ResponseTest01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// (이떄 파라미터를 넘겨줄수 있다.)
		//
		// 다름 서블릿이나 JSP로 제어가 넘어갈 때 URL주소는 변하지 않는다.
		// 그래서 클라이언트에게 페이지 주소를 숨길 떄 사용할 수 있으며,
		// redirect보다 성능이 좋다.
		// (웹 애플리케이션 내부에서만 접근이 가능하다.)

		// 이동되는 페이지로 원하는'데이터'를 넘기려면 request객체의
		// setAttribute()메서드를 이용하여 데이터를 세팅하고
		// 받는 쪽에서는 request객체의 getAttribute()메서드로 읽어온다.

		// forwarding 작업 순서
		// 1. 전달할 데이터가 있으면 setAttribute()메서드로 세팅한다.
		request.setAttribute("tel", "010-1111-1111");

		// 2. RequestDispatcher객체를 생성한다.
		// ==> 이때 이동할 페이지를 지정한다.

		RequestDispatcher re = request.getRequestDispatcher("/requestForwardTest.do");
